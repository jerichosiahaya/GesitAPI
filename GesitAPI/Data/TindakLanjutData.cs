...
namespace GesitAPI.Data
{
    public class TindakLanjutData : ITindakLanjut
    {
        ...
        public async Task<IEnumerable<TindakLanjut>> GetByRhaID(string idRha)
        {
            var result = await _db.TindakLanjuts.OrderByDescending(s => s.CreatedAt).Where(s => s.RhaId == Convert.ToInt32(idRha)).AsNoTracking().ToListAsync();
            return result;
        }
